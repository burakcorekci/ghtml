...
// build/dev/javascript/example_06_material_web/components/md_button.mjs
function render(label, on_click2) {
  return element2("md-filled-button", toList([on_click(on_click2())]), toList([text2(label)]));
}

// build/dev/javascript/example_06_material_web/components/md_checkbox.mjs
function render2(is_checked, on_change) {
  if (is_checked) {
    return element2("md-checkbox", toList([attribute2("checked", ""), on_click(on_change())]), toList([]));
  } else {
    return element2("md-checkbox", toList([on_click(on_change())]), toList([]));
  }
}

// build/dev/javascript/example_06_material_web/components/md_fab.mjs
function render3(icon, on_click2) {
  return element2("md-fab", toList([on_click(on_click2())]), toList([
    element2("md-icon", toList([attribute2("slot", "icon")]), toList([text2(icon)]))
  ]));
}

// build/dev/javascript/example_06_material_web/types.mjs
class ListItem extends CustomType {
  constructor(id, headline, supporting) {
    super();
    this.id = id;
...
// build/dev/javascript/example_06_material_web/components/md_list.mjs
function render4(items) {
  return element2("md-list", toList([]), toList([
    fragment2(index_map(items, (item, idx) => {
      return [
        to_string(idx),
        element2("md-list-item", toList([]), toList([
          span(toList([attribute2("slot", "headline")]), toList([text2(item.headline)])),
          span(toList([attribute2("slot", "supporting-text")]), toList([text2(item.supporting)]))
        ]))
      ];
    }))
  ]));
...
// build/dev/javascript/example_06_material_web/components/md_outlined_button.mjs
function render5(label, on_click2) {
  return element2("md-outlined-button", toList([on_click(on_click2())]), toList([text2(label)]));
}

// build/dev/javascript/example_06_material_web/components/md_switch.mjs
function render6(is_on, on_change) {
  if (is_on) {
    return element2("md-switch", toList([
      attribute2("selected", ""),
      on_click(on_change())
    ]), toList([]));
  } else {
    return element2("md-switch", toList([on_click(on_change())]), toList([]));
  }
}

// build/dev/javascript/example_06_material_web/components/md_text_button.mjs
function render7(label, on_click2) {
  return element2("md-text-button", toList([on_click(on_click2())]), toList([text2(label)]));
}

// build/dev/javascript/example_06_material_web/components/md_textfield.mjs
function render8(value2, label, on_input2) {
  return element2("md-outlined-text-field", toList([
    value(value2),
    attribute2("label", label),
    on_input(on_input2)
  ]), toList([]));
}

// build/dev/javascript/example_06_material_web/app.mjs
var FILEPATH = "src/app.gleam";

class Model extends CustomType {
  constructor(input_value, checkbox_checked, switch_on, fab_clicks) {
    super();
    this.input_value = input_value;
    this.checkbox_checked = checkbox_checked;
    this.switch_on = switch_on;
    this.fab_clicks = fab_clicks;
  }
}

class UpdateInput extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class ToggleCheckbox extends CustomType {
}

class ToggleSwitch extends CustomType {
}

class IncrementFab extends CustomType {
}

class ButtonClicked extends CustomType {
}
function init(_) {
  return new Model("", false, false, 0);
}
function update2(model, msg) {
  if (msg instanceof UpdateInput) {
    let value2 = msg[0];
    return new Model(value2, model.checkbox_checked, model.switch_on, model.fab_clicks);
  } else if (msg instanceof ToggleCheckbox) {
    return new Model(model.input_value, !model.checkbox_checked, model.switch_on, model.fab_clicks);
  } else if (msg instanceof ToggleSwitch) {
    return new Model(model.input_value, model.checkbox_checked, !model.switch_on, model.fab_clicks);
  } else if (msg instanceof IncrementFab) {
    return new Model(model.input_value, model.checkbox_checked, model.switch_on, model.fab_clicks + 1);
  } else {
    return model;
  }
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Material Web Components Example")])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Buttons")])),
      p(toList([]), toList([
        text3("Material Web provides three button variants: filled, outlined, and text.")
      ])),
      div(toList([class$("button-row")]), toList([
        render("Filled", () => {
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Text Field")])),
      p(toList([]), toList([
        text3("Material Web text field with standard @input event. Uses fn(String) -> msg handler.")
      ])),
      div(toList([class$("input-section")]), toList([
        render8(model.input_value, "Your Name", (var0) => {
          return new UpdateInput(var0);
        }),
        div(toList([class$("value-display")]), toList([text3("Current value: " + model.input_value)]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Checkbox")])),
      p(toList([]), toList([
        text3("Material Web checkbox with @click event and boolean 'checked' attribute.")
      ])),
      div(toList([class$("toggle-row")]), toList([
        render2(model.checkbox_checked, () => {
          return new ToggleCheckbox;
        }),
        span(toList([class$("status")]), toList([
          text3((() => {
            let $ = model.checkbox_checked;
            if ($) {
              return "Checked";
            } else {
              return "Unchecked";
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Switch")])),
      p(toList([]), toList([
        text3("Material Web switch with @click event and boolean 'selected' attribute.")
      ])),
      div(toList([class$("toggle-row")]), toList([
        render6(model.switch_on, () => {
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("List")])),
      p(toList([]), toList([
        text3("Material Web list using {#each} to render items with slots for headline and supporting text.")
      ])),
      div(toList([class$("list-section")]), toList([
        render4(toList([
          new ListItem("1", "First Item", "Supporting text for the first item"),
          new ListItem("2", "Second Item", "Supporting text for the second item"),
          new ListItem("3", "Third Item", "Supporting text for the third item")
        ]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Floating Action Button")])),
      p(toList([]), toList([
        text3("FAB with icon slot. Click the FAB in the bottom-right corner. Clicks: "),
        text3(to_string(model.fab_clicks))
      ]))
    ])),
    div(toList([class$("fab-container")]), toList([render3("add", () => {
      return new IncrementFab;
    })]))
  ]));
}
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "app", 18, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 446, end: 495, pattern_start: 457, pattern_end: 462 });
  }
  return;
}

// build/dev/javascript/example_06_material_web/example_06_material_web.mjs
function main2() {
  return main();
}

// .lustre/build/example_06_material_web.mjs
main2();
