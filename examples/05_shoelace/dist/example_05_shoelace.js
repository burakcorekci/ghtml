...
// build/dev/javascript/example_05_shoelace/components/sl_button.mjs
function render(label, variant, on_click2) {
  return element2("sl-button", toList([
    attribute2("variant", variant),
    on_click(on_click2())
  ]), toList([text2(label)]));
}

// build/dev/javascript/example_05_shoelace/components/sl_card.mjs
function render2(title, body, image_src) {
  return element2("sl-card", toList([class$("card")]), toList([
    img(toList([
      attribute2("slot", "image"),
      src(image_src),
      alt("")
    ])),
    strong(toList([]), toList([text2(title)])),
    p(toList([]), toList([text2(body)]))
  ]));
}

// build/dev/javascript/example_05_shoelace/components/sl_checkbox.mjs
function render3(label, is_checked, on_change) {
  if (is_checked) {
    return element2("sl-checkbox", toList([
      attribute2("checked", ""),
      on("sl-change", on_change)
    ]), toList([text2(label)]));
  } else {
    return element2("sl-checkbox", toList([on("sl-change", on_change)]), toList([text2(label)]));
  }
}

// build/dev/javascript/example_05_shoelace/components/sl_dialog.mjs
function render4(title, is_open, on_close_decoder, on_close_click) {
  if (is_open) {
    return element2("sl-dialog", toList([
      ...
      p(toList([]), toList([text2("This is a Shoelace dialog component.")])),
...
// build/dev/javascript/example_05_shoelace/components/sl_input.mjs
function render5(value2, label, placeholder2, on_input) {
  return element2("sl-input", toList([
    value(value2),
    attribute2("label", label),
    placeholder(placeholder2),
    on("sl-input", on_input)
  ]), toList([]));
}

// build/dev/javascript/example_05_shoelace/types.mjs
class Option extends CustomType {
  constructor(value2, label) {
    super();
    this.value = value2;
    this.label = label;
  }
...
// build/dev/javascript/example_05_shoelace/components/sl_select.mjs
function render6(label, options, selected, on_change) {
  return element2("sl-select", toList([
    attribute2("label", label),
    value(selected),
    on("sl-change", on_change)
  ]), toList([
    fragment2(index_map(options, (opt, idx) => {
      return [
        to_string(idx),
        element2("sl-option", toList([value(opt.value)]), toList([text2(opt.label)]))
      ];
    }))
  ]));
}

// build/dev/javascript/example_05_shoelace/app.mjs
var FILEPATH = "src/app.gleam";

class Model extends CustomType {
  constructor(input_value, checkbox_checked, dialog_open, selected_color) {
    super();
...
class OpenDialog extends CustomType {
}

class CloseDialog extends CustomType {
}

class SelectColor extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init(_) {
  return new Model("", false, false, "blue");
}
function update2(model, msg) {
  if (msg instanceof UpdateInput) {
    let value2 = msg[0];
    return new Model(value2, model.checkbox_checked, model.dialog_open, model.selected_color);
  } else if (msg instanceof ToggleCheckbox) {
    return new Model(model.input_value, !model.checkbox_checked, model.dialog_open, model.selected_color);
  } else if (msg instanceof OpenDialog) {
    return new Model(model.input_value, model.checkbox_checked, true, model.selected_color);
  } else if (msg instanceof CloseDialog) {
    return new Model(model.input_value, model.checkbox_checked, false, model.selected_color);
  } else {
    let color = msg[0];
    return new Model(model.input_value, model.checkbox_checked, model.dialog_open, color);
  }
}
...
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Shoelace Components Example")])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Buttons")])),
      p(toList([]), toList([
        text3("Shoelace buttons with different variants. Notice how custom elements use element() instead of html.button().")
      ])),
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Input")])),
      p(toList([]), toList([
        text3('Shoelace input with @sl-input event. Custom events use event.on("sl-input", decoder).')
      ])),
      div(toList([class$("input-section")]), toList([
        render5(model.input_value, "Your Name", "Enter your name...", decode_input_value((var0) => {
    ...
    div(toList([class$("section")]), toList([
      ...
      p(toList([]), toList([
        text3("Shoelace checkbox with @sl-change event and boolean 'checked' attribute.")
      ])),
      div(toList([class$("checkbox-row")]), toList([
        render3("Enable notifications", model.checkbox_checked, success(new ToggleCheckbox)),
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Select")])),
      p(toList([]), toList([
        text3("Shoelace select dropdown with {#each} to render options dynamically.")
      ])),
      div(toList([class$("select-section")]), toList([
        render6("Favorite Color", toList([
          new Option("red", "Red"),
          new Option("green", "Green"),
          new Option("blue", "Blue"),
          new Option("purple", "Purple")
        ]), model.selected_color, decode_input_value((var0) => {
          return new SelectColor(var0);
        })),
        div(toList([class$("value-display")]), toList([text3("Selected: " + model.selected_color)]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Card")])),
      p(toList([]), toList([
        text3('Shoelace card with slot="image" for image placement. Slots are just regular attributes.')
      ])),
      render2("Lustre + Shoelace", "Web components integrate seamlessly with Lustre templates. Custom elements are automatically detected and rendered correctly.", "https://images.unsplash.com/photo-1559583985-c80d8ad9b29f?w=300&h=200&fit=crop")
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Dialog")])),
      p(toList([]), toList([
        text3("Click any button above to open the dialog. Uses @sl-hide event to detect when closed.")
      ])),
      render4("Example Dialog", model.dialog_open, success(new CloseDialog), () => {
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "app", 16, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 384, end: 433, pattern_start: 395, pattern_end: 400 });
  }
  return;
}

// build/dev/javascript/example_05_shoelace/example_05_shoelace.mjs
function main2() {
  return main();
}

// .lustre/build/example_05_shoelace.mjs
main2();
