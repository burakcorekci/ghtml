...
/// Generate Gleam code from a parsed template for the Lustre target.
pub fn generate(template: Template, source_path: String, hash: String) -> String {
  let filename = shared.extract_filename(source_path)
...
fn generate_static_attr(name: String, value: String) -> String {
  case find_attr_function(name) {
    Ok(func) -> func <> "(\"" <> shared.escape_string(value) <> "\")"
    Error(_) ->
      "attribute.attribute(\""
      <> name
      <> "\", \""
      <> shared.escape_string(value)
      <> "\")"
  }
}
...
fn generate_text_inline(content: String) -> String {
  let normalized = shared.normalize_whitespace(content)
  case shared.is_blank(normalized) {
    True -> ""
    False -> "text(\"" <> shared.escape_string(normalized) <> "\")"
  }
}
